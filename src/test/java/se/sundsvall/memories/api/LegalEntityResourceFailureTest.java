...
class LegalEntityResourceFailureTest {
	...
	@Test
	void searchLegalEntitiesWithInvalidMunicipalityId() {
		...
		assertThat(response.getViolations())
			...
			.containsExactlyInAnyOrder(tuple("searchLegalEntities.municipalityId", "not a valid municipality ID"));

		verifyNoInteractions(serviceMock);
	}

	@Test
	void getLegalEntityByIdWithInvalidMunicipalityId() {
		final var response = webTestClient.get()
		...
		assertThat(response.getViolations())
			.extracting(Violation::field, Violation::message)
			.containsExactlyInAnyOrder(tuple("getLegalEntity.municipalityId", "not a valid municipality ID"));
