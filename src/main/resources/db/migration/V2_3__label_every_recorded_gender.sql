-- Changes VW_MEMORY_OBJECTS (see V1_7 through V2_2) in one way; everything else is unchanged, and no table is
-- altered.
--
--   * GENDER now labels every row of the two branches that carry a KON column (PERSON, MANTAL): the stray values
--     V2_2 left as NULL — blank, '0', '3', birth dates from a shifted column — become 'Okänt' along with the rows
--     that spell it out. A value naming no gender is a gender the archive does not know, which is what Okänt says,
--     and the combined search's genderCounts now add up to the number of register rows the search matched.
--
-- With the ELSE in place, a NULL GENDER means one thing only: the row belongs to a type that records no gender at
-- all (every object type, and SJOMAN, which has no such column). The gender filter reads it that way — it leaves
-- those rows untouched instead of excluding them — so this view must be in place before that code ships, or the
-- stray-valued register rows would pass every gender selection.
