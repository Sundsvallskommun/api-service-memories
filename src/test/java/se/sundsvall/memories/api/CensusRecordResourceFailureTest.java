...
class CensusRecordResourceFailureTest {
	...
	@Test
	void searchCensusRecordsWithInvalidMunicipalityId() {
	...
	@Test
	void getCensusRecordByIdWithInvalidMunicipalityId() {
		final var response = webTestClient.get()
		...
		assertThat(response.getViolations())
			.extracting(Violation::field, Violation::message)
			.containsExactlyInAnyOrder(tuple("getCensusRecord.municipalityId", "not a valid municipality ID"));
