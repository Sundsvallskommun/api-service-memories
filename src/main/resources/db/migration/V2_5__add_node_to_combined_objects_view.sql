-- Changes VW_MEMORY_OBJECTS (see V1_7 through V2_3) in one way; everything else is unchanged, and no table is
-- altered.
--
--   * NODE_ID carries each object's NODEID — the archive node (arkiv, serie or volym) it was created in — so the
--     combined search can list what a node holds, the way the archive pages of the legacy site do. The registers
--     (PERSON, JURPERS, SJOMAN, MANTAL) are not placed in the tree and emit NULL, which the nodeId filter reads as
--     "in no node": selecting a node keeps the objects in it and nothing else.
--
-- Unlike the legacy tables this view is ours. Flyway is off in the real environments, so this has to be applied by
-- hand there before the code that reads NODE_ID ships.
