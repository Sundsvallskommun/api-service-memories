...
@RestController
@Validated
@RequestMapping("/{municipalityId}/census-records")
@Tag(name = "Census record", description = "Census record (mantal) search and retrieval")
@ApiResponses(value = {
...
class CensusRecordResource {
	...
	CensusRecordResource(final CensusRecordService censusRecordService) {
		this.censusRecordService = censusRecordService;
	}

	@GetMapping(produces = APPLICATION_JSON_VALUE)
	@Operation(summary = "Search census records", description = "Search census records (MANTAL) by name, birth year range and gender. Returns paginated results.")
	@ApiResponse(responseCode = "200", description = "Successful operation")
	ResponseEntity<PagedCensusRecordResponse> searchCensusRecords(
		@PathVariable @ValidMunicipalityId final String municipalityId,
		@Valid final CensusRecordParameters parameters) {

		return ok(censusRecordService.search(parameters));
	}
